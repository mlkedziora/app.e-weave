...
export default function ProjectInventoryModal({
  ...
  const MIN_Y_OFFSET = 22; // Adjust this to change the top limit (positive: lower min Y, negative: allow higher than topbar)
  const MIN_X_OFFSET = 400; // Adjust this to change the left limit (positive: more right, negative: allow left of sidebar)
  const MAX_Y_OFFSET = -33; // Adjust this for bottom limit (positive: allow lower, negative: restrict more)
  const MAX_X_OFFSET = -35; // Adjust this for right limit (positive: allow more right, negative: restrict more)
  const draggable = true; // Hardcoded as true
  ...
  useEffect(() => {
    ...
    if (draggable && innerRef.current) {
      ...
      if (initialMousePosition) {
        initX = initialMousePosition.x - width / 2;
        initY = initialMousePosition.y - height / 2;
      }

      // Apply limits to initial position
      initX = Math.max(sidebarWidth + MIN_X_OFFSET, Math.min(initX, window.innerWidth - width + MAX_X_OFFSET));
      initY = Math.max(topbarHeight + MIN_Y_OFFSET, Math.min(initY, window.innerHeight - height + MAX_Y_OFFSET));

      setPosition({ x: initX, y: initY });
      setUseAbsolute(true);
    }
  }, [draggable, initialMousePosition]);
  ...
  }, [dragging, draggable, rel, position]);
  ...
  const outerClass = `fixed inset-0 backdrop-blur-[3px] z-50 overscroll-none ${useAbsolute || !draggable ? '' : 'flex items-start justify-center'}`;
  const outerStyle = {
    opacity: fadeIn ? 1 : 0,
  ...
  const combinedInnerStyle = {
    ...(useAbsolute || !draggable ? {} : {}),
    ...(useAbsolute ? { position: 'absolute', left: position.x, top: position.y } : {}),
  };
  const combinedInnerClass = `bg-white p-0 rounded-lg shadow-md overflow-y-auto w-11/12 max-w-4xl max-h-[80vh] select-none overscroll-none ${useAbsolute || !draggable ? '' : 'mt-[1px]'}`;
    ...
    <div className={outerClass} style={outerStyle} onClick={onClose}>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          {/* Material List */}
          <ScrollablePanel className="space-y-4 flex-1 [--progress-bar-height:0.4rem] [--progress-fill-height:0.2rem] [--progress-bar-width:100%] [--progress-bg-color:#d4d4d4] [--progress-padding:0.155rem]">
            {filtered.length > 0 ? (
